...
function wp_rss_multi_importer_display($active_tab = '')
{
	...
	<div class="wrap">
		...
		if (isset($_GET['page']) && $_GET['page'] == 'wp_rss_multi_importer_admin' && $wprssmi_admin_options['dismiss_slug'] != "true") {
			...
			<div id="message" class="updated fade">
				...
				<form method="post" action="options.php">
					...
					<input type="submit" value="<?php _e("Dismiss This Message", 'wp-rss-multi-importer')?>"
						   name="submit">
				</form>
		...
		<h2 class="nav-tab-wrapper">
			<a href="?page=wp_rss_multi_importer_admin&tab=intro"
			   class="nav-tab <?php echo $active_tab == 'intro' ? 'nav-tab-active' : ''; ?>"><?php  _e("Overview", 'wp-rss-multi-importer')?></a>
			<a href="?page=wp_rss_multi_importer_admin&tab=items_list"
			   class="nav-tab <?php echo $active_tab == 'items_list' ? 'nav-tab-active' : ''; ?>"><?php  _e("Feeds", 'wp-rss-multi-importer')?></a>
			<a href="?page=wp_rss_multi_importer_admin&tab=category_options"
			   class="nav-tab <?php echo $active_tab == 'category_options' ? 'nav-tab-active' : ''; ?>"><?php  _e("Categories", 'wp-rss-multi-importer')?></a>
			<a href="?page=wp_rss_multi_importer_admin&tab=setting_options"
			   class="nav-tab <?php echo $active_tab == 'setting_options' ? 'nav-tab-active' : ''; ?>"><?php  _e("Shortcode Settings", 'wp-rss-multi-importer')?></a>
			<a href="?page=wp_rss_multi_importer_admin&tab=feed_to_post_options"
			   class="nav-tab <?php echo $active_tab == 'feed_to_post_options' ? 'nav-tab-active' : ''; ?>"><?php  _e("Feed to Post Settings", 'wp-rss-multi-importer')?></a>
			<a href="?page=wp_rss_multi_importer_admin&tab=shortcode_parameters"
			   class="nav-tab <?php echo $active_tab == 'shortcode_parameters' ? 'nav-tab-active' : ''; ?>"><?php  _e("Shortcode Parameters", 'wp-rss-multi-importer')?></a>
			<a href="?page=wp_rss_multi_importer_admin&tab=template_options"
			   class="nav-tab <?php echo $active_tab == 'template_options' ? 'nav-tab-active' : ''; ?>"><?php  _e("Template Options", 'wp-rss-multi-importer')?></a>

			<a href="?page=wp_rss_multi_importer_admin&tab=feed_options"
			   class="nav-tab <?php echo $active_tab == 'feed_options' ? 'nav-tab-active' : ''; ?>"><?php  _e("Export RSS", 'wp-rss-multi-importer')?></a>
...
add_action('wpseo_canonical', 'fixCanonicalForImporter', 10, 3);

function fixCanonicalForImporter($page_path) {
	global $post;
	if($post) {
		$rssmi_source_link = get_post_meta($post->ID, 'rssmi_source_link', true);
		if($rssmi_source_link)
			return $rssmi_source_link;
	}
